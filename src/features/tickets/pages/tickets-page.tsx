...
import { CustomerReportSheet } from '@/features/reports';
import {
...
function canCancelTicket(status: Ticket['status']) {
  ...
  const cancelTicketMutation = useMutation({
    mutationFn: async (ticketId: number) =>
      updateStatus(ticketId, {
        status: 'cancelled',
        comment: 'Ticket cancelado por el solicitante',
      }),
    onSuccess: (updatedTicket) => {
      syncUpdatedTicketCaches(queryClient, updatedTicket);
      invalidateTicketCaches(queryClient);
      toast.success('La solicitud fue cancelada correctamente');
    },
    onError: () => {
  ...
  return (
    <>
        ...
        <section className="lively-hero rounded-[var(--radius-panel)] px-6 py-7 sm:px-8 sm:py-9">
          <div className="relative z-10">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
              Revisa el estado de tus solicitudes, consulta el resumen del trabajo realizado y crea una nueva cuando lo necesites.
            </p>
            <div className="mt-6">
          ...
          <CardContent className="space-y-5 px-6 pb-6 pt-6 sm:px-8 sm:pb-8">
            {isLoading ? (
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                {tickets.map((ticket) => {
                  const isCancellingThisTicket =
                    cancelTicketMutation.isPending &&
                    cancelTicketMutation.variables === ticket.id;
                    ...
                    <Card key={ticket.id} className="ticket-record-card rounded-[var(--radius-panel)]">
                      <CardHeader className="px-5 pb-0 pt-5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {ticket.report?.id ? <Badge variant="secondary">Con reporte</Badge> : null}
                        </div>
                      </CardHeader>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {canCancelTicket(ticket.status) ? (
                            <Button
                              ...
                              variant="destructive"
                              size="sm"
                              disabled={isCancellingThisTicket}
                              onClick={() => setTicketToCancel(ticket)}
                            >
                              <Ban className="mr-2 h-4 w-4" />
                              {isCancellingThisTicket ? 'Cancelando...' : 'Cancelar ticket'}
                            </Button>
                          ) : null}
                        </div>
      ...
      <ConfirmDialog
        ...
        description={
          ticketToCancel
            ? `Se cancelara el ticket "${ticketToCancel.title}". Esta accion cambiara su estado a cancelado.`
            : ''
        }
        actionLabel="Cancelar ticket"
        onConfirm={() => {
          if (ticketToCancel) {
      ...
      <CustomerReportSheet
        open={Boolean(reportTicket)}
        ticketId={reportTicket?.id ?? null}
        ticketTitle={reportTicket?.title}
        onOpenChange={(open) => {
          if (!open) {
